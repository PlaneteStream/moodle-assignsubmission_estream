...
?>
		...
		<div style="text-align: center;">
         <iframe width="90%" height="700px" allow="camera; microphone; display-capture; local-network-access *" frameborder="0" src="<?php echo $baseurl; ?>/VLE/Moodle/Default.aspx?sourceid=11&inlinemode=moodle&delta=<?php echo $delta?>&checksum=<?php echo $checksum; ?>&itemcdid=<?php echo $itemcdid; ?>&assign=<?php echo ((string)$PAGE->pagetype == 'mod-assign-editsubmission' ? "true" : "false"); ?>&assignmoodle=<?php echo ((string)$PAGE->pagetype == 'mod-assign-submission-estream-upload' ? "true" : "false"); ?>&ticket=<?php echo urlencode($authticket); ?>&murl=<?php echo $CFG->wwwroot.'/mod/assign/submission/estream/callback.php&amp;title='.urlencode($itemtitle).'&amp;desc='.urlencode($itemdesc).'&amp;cid='.urlencode($itemcid).'&amp;aid='.urlencode($itemaid).'&amp;uid='.urlencode($itemuid).'&amp;cdid='.$itemcdid;?>"></iframe>
        </div>
<?php
    }
}
